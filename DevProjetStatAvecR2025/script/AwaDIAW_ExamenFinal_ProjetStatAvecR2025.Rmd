...
flextable(data.frame(Contenu = c("Agence nationale de la Statistique et de la démographie"))) %>% 
...
flextable(data.frame(Contenu = c("Ecole nationale de la Statistique et de l'Analyse économique Pierre Ndiaye"))) %>%
  delete_part(part = "header") %>% 
  border_remove() %>% 
  font(fontname = "Times New Roman", part = "all") %>% 
  fontsize(size = 12, part = "all") %>% 
...
```{r image_theme, echo=FALSE, out.width='30%', out.height='40px', fig.align='center'}
...
Dans un contexte où la donnée est devenue un levier stratégique pour comprendre et agir sur les réalités socio-économiques, la maîtrise des outils d’analyse statistique est une compétence incontournable pour tout statisticien. Le logiciel R s’impose aujourd’hui comme une référence dans le monde de la data science grâce à sa puissance, sa flexibilité et sa vaste communauté. Il permet non seulement d’analyser des données de manière rigoureuse, mais aussi d’automatiser des processus, de générer des visualisations percutantes, et d'assurer la reproductibilité des travaux scientifiques.

C’est dans cette dynamique que s’inscrit le cours de projet statistique avec R, dispensé au sein du cycle long ISE1, qui vise à initier les étudiants à des pratiques concrètes d’analyse de données en mobilisant les outils modernes du langage R. Ce projet d’examen en constitue l’aboutissement.

Je tiens à adresser mes sincères remerciements à Monsieur Aboubacar HEMA pour la qualité de son enseignement, sa disponibilité et ses conseils méthodologiques. Son accompagnement, ainsi que les ressources partagées via son compte GitHub, ont été précieux tout au long de ce travail. Mes remerciements vont également à mes camarades pour les échanges constructifs et la collaboration enrichissante que nous avons eue depuis le début de notre formation.

Je précise enfin que toute erreur, omission ou maladresse contenue dans ce rapport relève exclusivement de ma responsabilité.
...
Ce rapport présente une analyse statistique détaillée de la sécurité alimentaire des ménages au Tchad, à partir des enquêtes EHCVM menées en 2022 et 2023. Nous avons procédé à la vérification de la consistance des bases de données, calculé des indicateurs clés de sécurité alimentaire et de résilience — tels que le Score de Consommation Alimentaire (SCA), l’Indice Réduit des Stratégies de Survie (rCSI), l’Indice des Stratégies d’Adaptation des Moyens d’Existence (LhCSI‑FS), le Score de Diversité Alimentaire (HDDS), le Score de Résilience Auto‑évaluée (SERS), et le Régime Alimentaire Minimum Acceptable (MAD) pour les enfants de 6 à 23 mois — et réalisé des analyses comparatives selon le genre du chef de ménage. Des cartographies spatiales au niveau des régions et départements illustrent les disparités géographiques. Les résultats mettent en évidence des zones particulièrement vulnérables et suggèrent des pistes d’interventions ciblées pour renforcer la résilience alimentaire au Tchad.

\newpage

# Introduction

La sécurité alimentaire demeure un enjeu majeur au Tchad, confronté à des chocs climatiques, des conflits et des faiblesses structurelles. Cette étude exploite les données de l’EHCVM 2022-2023. 

## Matériels et méthodes

### Données

- **Base_Principale** : comprend les modules SCA, rCSI, LhCSI, HDDS, SERS et les variables socio‑démographiques.  
- **Base_MAD** : contient les informations du module MAD pour les enfants de 6 à 23 mois.
- **tchad** : Shapefile contenant les géométries des régions et département du tchad

## Logiciels

L’analyse a été réalisée sous **R 4.2**, avec les packages suivants :`tidyverse`, pour la manipulation et la visualisation des données [@tidyverse2025] ; `janitor`, pour le nettoyage des noms de variables et le repérage rapide d’anomalies [@janitor2025] ; `gtsummary`, pour la génération de tableaux statistiques lisibles et adaptés à la rédaction de rapports [@gtsummary2025] ; `officer` et `officedown`, pour l'automatisation de la génération de rapports Word structurés, interactifs et présentables [@rmarkdown2025]; entre autres.

## Plan d’analyse

- Vérification de la consistance des bases de données ;  
- Analyse socio‑démographique et calcul des indicateur (SCA, rCSI, LhCSI‑FS, HDDS, SERS et MAD) ;  
- Analyse comparative selon le genre du chef de ménage ;  
- Conclusion.
...
L'analyse de consistance est une étape cruciale pour s'assurer de la qualité des données avant de procéder à des analyses plus poussées.Nous allons nettoyer les bases avec janitor avant de procéder à leur fusion afin de garantir une jointure correcte. Fusionner sans nettoyage peut entraîner des erreurs, des doublons ou la perte d’informations.
...
Dans les bases, la valeur 888 est utilisée pour indiquer que la réponse est « ne sait pas ». Afin d'éviter que cette valeur soit interprétée comme une donnée réelle lors des analyses statistiques ou graphiques, dans la suite du nettoyage, elle va être convertie en NA, la représentation standard des valeurs manquantes en R.
...
# Recodage des valeurs 888 en NA dans la base principale

# Vars num (ce sont elles qui peuvent prendre 888)
num_vars_principal <- principal_dataset%>% 
  select(where(is.numeric)) %>% 
  names()

# Recodage
principal_dataset_cleaned <- principal_dataset %>% 
  mutate(across(all_of(num_vars_principal), ~ na_if(., 888)))
```

### 3. Visualisation des missings (valeurs manquantes)
...
#principal
na_count_pdataset <- principal_dataset_cleaned  %>%
  summarise(across(everything(), ~sum(is.na(.)))) %>%
  pivot_longer(everything(), names_to = "variable", values_to = "missing_count") %>%
  filter(missing_count > 0) %>%
  arrange(desc(missing_count))
...
```{r visualisation_missings_mad,fig.cap="Visualisation des valeurs manquantes de la base MAD"}
...
na_count_maddataset %>%
  ...
  geom_col(fill = "brown") +
  coord_flip() +
  labs(caption = "Source : EHCVM 2022-2023, Calculs propres",
       x = "Variables de la base MAD", y = "Nombre de NA") +
  theme_minimal()
```

```{r visualisation_missings_principal,fig.cap="Visualisation des valeurs manquantes de la base principale"}
...
na_count_pdataset %>%
  ...
  coord_flip() +
  labs(caption = "Source : EHCVM 2022-2023, Calculs propres",
       x = "Variables de la base principale", y = "Nombre de NA") +
  theme_minimal()
```
...
Nous avons utilisé right_join() car principal_dataset constitue la base principale d’analyse, contenant  `r nrow(principal_dataset)` observations. Nous souhaitons y ajouter les informations complémentaires de mad_dataset2 `r nrow(mad_dataset_cleaned)` observations, sans perdre aucune unité statistique présente dans la base principale.

```{r combine-data}
# Combinaison des données avec la variable 'id'
combined_dataset <- right_join(mad_dataset_cleaned , principal_dataset_cleaned, by = "id")
```
...
# Créer la table avec toutes les variables correctement séparées
tableau_description <- data.frame(
  Variable = c(
    "hh_size", "hhh_sex", "hhh_age", "hhh_edu", "hh_source_income",
    "admin1name", "admin2name",
    "hh_size05m", "hh_size23m", "hh_size59m", "hh_size5114m", 
...
tableauflex_analyse_sociodemo <- tableau_description %>%
  ...
  flextable::set_header_labels(values = list(
    Variable = "Variables socio-démographiques",
    Description = "Description"
  )) %>%
  flextable::set_caption(
    caption = "Analyse socio-démographique des ménages",
...
# 1. Convertir les colonnes "labelled" en facteurs
...
# 2. Créer le tableau avec tbl_summary
tableau_analyse_sociodemo <- combined_dataset %>%
  tbl_summary(
    ...
    label = list(
      ...
      hh_size05m ~ "Garçons < 5 ans",
      hh_size23m ~ "Garçons 2-3 ans",
      hh_size59m ~ "Garçons 5-9 ans",
      hh_size5114m ~ "Garçons 5-14 ans",
      hh_size1549m ~ "Hommes 15-49 ans",
      hh_size5064m ~ "Hommes 50-64 ans",
      hh_size65above_m ~ "Hommes 65 ans et +",
      hh_size05f ~ "Filles < 5 ans",
      hh_size23f ~ "Filles 2-3 ans",
      hh_size59f ~ "Filles 5-9 ans",
      hh_size5114f ~ "Filles 5-14 ans",
      hh_size1549f ~ "Femmes 15-49 ans",
      hh_size5064f ~ "Femmes 50-64 ans",
      hh_size65above_f ~ "Femmes 65 ans et +"
    ),
    missing = "no"
  ) %>%
  add_n() %>%                                  # Ajout de la taille de l'échantillon
  modify_header(label = "Variables socio-démographiques") %>%
  bold_labels() %>%
  italicize_levels()

# 3. Conversion en flextable avec légende automatique
as_flex_table(tableau_analyse_sociodemo) %>%
  set_caption(
    caption = "Caractéristiques socio-démographiques des ménages tchadiens",
    autonum = officer::run_autonum(seq_id = "tab")
  )
```
...
flextable(fcs_table) %>%
  ...
  set_caption("Tableau illustrant les poids attribués",
...
Ici, on crée deux variables catégorielles à partir du score FCS selon différents seuils.
...
flextable(seuils_fcs) %>%theme_booktabs() %>%
 set_caption("Grille de classification du FCS selon deux jeux de seuils",
 autonum = run_autonum(seq_id = "tab")) %>%
  autofit()
```
...
table_21_35 <- flextable(resume_fcs_21_35) %>%
  colformat_num(j = "Pourcentage", suffix = "%") %>%set_caption("Classification des ménages selon les seuils FCS 21/35",
...
table_28_42 <- flextable(resume_fcs_28_42) %>%
  colformat_num(j = "Pourcentage", suffix = "%") %>% set_caption("Classification des ménages selon les seuils FCS 28/42",
 autonum = run_autonum(seq_id = "tab")) %>%theme_booktabs() %>%
  autofit()
table_28_42
```
...
Ici, nous allons assurer une certaine harmonie entre les deux bases en faisant la jointure par code plutot que par nom de niveau administratif afin de réduire de nombre de N/A dans les calculs stratifié (région et département).
...
ggplot(combined_sf_par_region) +
  ...
  theme_minimal() +
  labs(
    caption = "Source : EHCVM 2022-2023, Calculs propres"
...
r_csi__vars <- combined_dataset%>%
  dplyr::select(matches("^r_csi_"))
```

Les variables composant le RCSI sont : `r colnames(r_csi__vars)`

### b. Calcul du score RCSI


```{r score-rcsi}
# Calcul du score rcsi dans la base combined_dataset
combined_dataset <- combined_dataset %>%
  mutate(
...
```{r score-rcsi_pour_representation}
...
flextable(rcsi_table) %>%
  set_header_labels(
    ...
    Poids = "Poids RCSI"
  ) %>% set_caption("Poids pour le calcul du RCSI",
 autonum = run_autonum(seq_id = "tab"))%>%
  autofit() %>%
  theme_booktabs()
```
...
flextable(lhcsi_table) %>%
  ...
  theme_booktabs() %>% set_caption("Tableau descriptif des variables LhCSI",
 autonum = run_autonum(seq_id = "tab")) %>%
  autofit()
```
...
\]
```{r calculs_lhcsi_par_hh}
combined_dataset <- combined_dataset %>%
  mutate(
    stress_hh = if_any(starts_with("lh_csi_stress"), ~ . == 3),
    crise_hh = if_any(starts_with("lh_csi_crisis"), ~ . == 3),
    urgence_hh = if_any(starts_with("lh_csi_emergency"), ~ . == 3)
  )
```

```{r calculs_lhcsi_par_annee}
principal_dataset_lhsi_par_annee <- combined_sf %>%
  mutate(
...
proportions_lhcsi <- principal_dataset_lhsi_par_annee %>%
  group_by(year) %>%
  summarise(
...
En 2022, la proportion de ménages en situation de stress est de `r round(proportions_lhcsi$prop_stress[proportions_lhcsi$year == 2022] * 100, 1)` %, en situation de crise `r round(proportions_lhcsi$prop_crise[proportions_lhcsi$year == 2022] * 100, 1)` %, 
et en urgence `r round(proportions_lhcsi$prop_urgence[proportions_lhcsi$year == 2022] * 100, 1)` %.

En 2023, ces proportions sont respectivement de `r round(proportions_lhcsi$prop_stress[proportions_lhcsi$year == 2023] * 100, 1)` %, `r round(proportions_lhcsi$prop_crise[proportions_lhcsi$year == 2023] * 100, 1)` %, et `r round(proportions_lhcsi$prop_urgence[proportions_lhcsi$year == 2023] * 100, 1)` %.


### c. Représentation spatiale (region et departement) des strategies d’adaptation par année

*Représentation régionale des strategies d’adaptation par année*

```{r calculs_lhcsi_region}
# Calcul des indicateurs à partir de combined_dataset

# Clean the dataset and compute indicators
adaptation_summary_reg <- combined_dataset %>%
  filter(!is.na(year), year %in% c(2022, 2023)) %>%  # keep only valid years
  mutate(
    stress = if_any(starts_with("lh_csi_stress"), ~ . == 3),
    crise = if_any(starts_with("lh_csi_crisis"), ~ . == 3),
    urgence = if_any(starts_with("lh_csi_emergency"), ~ . == 3)
...
# Create full region × year grid for 2022 & 2023
all_regions <- unique(tchad$admin1Pcod)  # from shapefile to ensure complete regions
years <- c(2022, 2023)

region_year_grid <- expand_grid(adm1_ocha = all_regions, year = years)

# Merge grid with summary (ensures all combos exist)
adaptation_complete <- region_year_grid %>%
  left_join(adaptation_summary_reg, by = c("adm1_ocha", "year"))

# Join with spatial data
adaptation_par_region <- tchad %>%
  left_join(adaptation_complete, by = c("admin1Pcod" = "adm1_ocha"))
...
```{r representation_lhcsi_reg3,fig.cap="Proportion de ménages en situation d'urgence par région"}
# visualisation spatiale avec ggplot2
ggplot(adaptation_par_region) +
...
*Représentation spatiale départementale des strategies d’adaptation par année*

```{r calculs_lhcsi_dep}
# Clean the dataset and compute indicators
adaptation_summary_dep <- combined_dataset %>%
  filter(!is.na(year), year %in% c(2022, 2023)) %>%  # keep only valid years
  mutate(
    stress = if_any(starts_with("lh_csi_stress"), ~ . == 3),
    crise = if_any(starts_with("lh_csi_crisis"), ~ . == 3),
  ...
  summarise(
    prop_stress_dep = mean(stress, na.rm = TRUE),
    prop_crise_dep = mean(crise, na.rm = TRUE),
    prop_urgence_dep = mean(urgence, na.rm = TRUE),
    .groups = "drop"
  )

# Create full dep × year grid for 2022 & 2023
all_dep <- unique(tchad$admin2Pcod)  # from shapefile to ensure complete deps
years <- c(2022, 2023)

dep_year_grid <- expand_grid(adm2_ocha = all_dep, year = years)

# Merge grid with summary (ensures all combos exist)
adaptation_complete_dep <- dep_year_grid %>%
  left_join(adaptation_summary_dep, by = c("adm2_ocha", "year"))

# Join with spatial data
adaptation_par_dep <- tchad %>%
  left_join(adaptation_complete_dep, by = c("admin2Pcod" = "adm2_ocha"))

```

- Stress + Département

```{r representation_lhcsi_dep1,fig.cap="Proportion de ménages en situation de stress par département"}
ggplot(adaptation_par_dep) +
  geom_sf(aes(fill = prop_stress_dep), color = "white") +
  facet_wrap(~year) +
  scale_fill_viridis_c(name = "Stress (%)",
...
```{r representation_lhcsi_dep2,fig.cap="Proportion de ménages en situation de crise par département"}
# visualisation spatiale avec ggplot2
ggplot(adaptation_par_dep) +
  geom_sf(aes(fill = prop_crise_dep), color = "white") +
  facet_wrap(~year) +
  scale_fill_viridis_c(name = "crise (%)", labels = scales::percent,
                       na.value = "gray80") +
  labs(subtitle = "Années 2022 et 2023",
...
```{r representation_lhcsi_dep3,fig.cap="Proportion de ménages en situation d'urgence par département"}
# visualisation spatiale avec ggplot2
ggplot(adaptation_par_dep) +
  geom_sf(aes(fill = prop_urgence_dep), color = "white") +
  facet_wrap(~year) +
  scale_fill_viridis_c(name = "urgence (%)", labels = scales::percent,
                       na.value = "gray80") +
  labs(subtitle = "Années 2022 et 2023",
...
#  Somme brute des scores (sans pondération)
combined_dataset <- combined_dataset %>%
  mutate(
    sers_raw_hh = rowSums(across(all_of(sers_vars)), na.rm = TRUE)
  )

# Étape 2 : Normalisation min-max vers une échelle de 0 à 100
sers_min_hh <- min(combined_sf$sers_raw_hh, na.rm = TRUE)
sers_max_hh <- max(combined_sf$sers_raw_hh, na.rm = TRUE)

combined_dataset <- combined_dataset %>%
  mutate(
    sers_score_hh = 100 * (sers_raw_hh - sers_min_hh) / (sers_max_hh - sers_min_hh)
  )
```

```{r calcul_sers_representation}
...
combined_dataset <- combined_dataset %>%
  mutate(
    nb_groupes_alim_hh = rowSums(select(., all_of(vars_mad_alim)) == 1, na.rm = TRUE)
  )
```
...
combined_dataset <- combined_dataset %>%
  mutate(
    ddm_hh = case_when(
      is.na(nb_groupes_alim_hh) ~ NA,         # Cas manquant
      nb_groupes_alim_hh >= 5 ~ 1,            # Respecte la DDM
...
combined_dataset <- combined_dataset %>%
  mutate(
    ...
    mad = case_when(
      allaitement == 1 & ddm_hh == 1 & mmf == 1 ~ 1,
      allaitement == 0 | ddm_hh == 0 | mmf == 0 ~ 0,
      TRUE ~ NA_integer_
    )
  )
...
### d. Statistiques descriptives de cette variable suivant le sexe du chef de menage, l’année

```{r tableau_mad_sex_annee}
...
as_flex_table(tbl_mad_par_annee) %>%
  set_caption("Présence d'un régime MAD suivant l'année et le sexe du chef de ménage",
 autonum = run_autonum(seq_id = "tab"))%>%
  autofit()
```
...
# Définir le thème compact pour les tableaux
set_gtsummary_theme(theme_gtsummary_compact())

# S'assurer que les colonnes catégorielles sont bien converties
combined_dataset <- combined_dataset %>%
  mutate(
    hhh_sex = haven::as_factor(hhh_sex)  # Genre du chef de ménage
  )

# Sélection des variables à inclure dans l'analyse comparative
vars_a_comparer <- c(
  "fcs_score", "hdds_score", "sers_raw_hh",
  "rcsi_score", "mmf", "nb_groupes_alim_hh", 
  "urgence_hh", "stress_hh", "crise_hh"
)

# Création du tableau comparatif par genre du chef de ménage
comparaison_genre <- combined_dataset %>%
  select(all_of(c("hhh_sex", vars_a_comparer))) %>%
  tbl_summary(
    by = hhh_sex,  # Comparaison par sexe du chef de ménage
    statistic = list(
      all_continuous() ~ "{mean} ({sd})",  # Moyenne (écart-type)
      all_categorical() ~ "{n} ({p}%)"     # Effectif (pourcentage)
    ),
    missing = "no",  # Ne pas afficher les valeurs manquantes comme modalité
    label = list(
      fcs_score ~ "Score de consommation alimentaire (FCS)",
      rcsi_score ~ "Score réduit d'adoption de stratégies (rCSI)",
      hdds_score ~ "Score de diversité alimentaire (HDDS)",
      sers_raw_hh ~ "Score de stratégies d'adaptation (SERS)",
      mmf ~ "Fréquence alimentaire minimale (MMF)",
      nb_groupes_alim_hh ~ "Nombre de groupes alimentaires",
      urgence_hh ~ "Situation d'urgence",
      stress_hh ~ "Situation de stress",
      crise_hh ~ "Situation de crise"
    )
  ) %>%
  add_n() %>%  # Ajouter le nombre total d'observations
  bold_labels() %>%
  italicize_levels()


# Affichage en tableau Word via flextable (sans utiliser cardx)
table_finale <- as_flex_table(comparaison_genre) %>%
  set_caption("Analyse comparative des indicateurs de sécurité alimentaire selon le genre du chef de ménage",
 autonum = run_autonum(seq_id = "tab"))%>%
  autofit() %>%
  width(j = 1:4, width = c(2.5, 1.5, 1.5, 5)) %>%  # Ajuste les colonnes si besoin
  align(align = "center", part = "all") %>%
...
L’échantillon est composé de 4006 ménages dirigés par une femme et 5073 par un homme, soit une répartition relativement équilibrée.

                          

##  Interprétation : Indicateurs de consommation alimentaire

```{r interp_global_2}

# Créer le tableau de résumé
tableau_comparatif <- tribble(
  ~Indicateur,                                  ~Femme,        ~Homme,        ~Interprétation,
  "FCS (Score de consommation alimentaire)",    "59 (±27)",    "61 (±25)",    "Légèrement plus élevé chez les hommes, suggérant une meilleure consommation alimentaire en moyenne.",
  "HDDS (Diversité alimentaire)",               "6.98 (±2.56)", "7.10 (±2.47)", "Légèrement plus diversifiée dans les ménages dirigés par un homme.",
  "MMF (Fréquence alimentaire minimale)",       "262 (34%)",   "339 (32%)",   "Proportion comparable entre les deux genres.",
  "Nombre de groupes alimentaires",             "0.63 (±1.91)", "0.71 (±2.09)", "Légère diversité alimentaire supérieure chez les hommes."
)

# Afficher le tableau avec flextable
flextable(tableau_comparatif) %>%
  set_caption("Comparaison globale des indicateurs de sécurité alimentaire selon le genre du chef de ménage",
 autonum = run_autonum(seq_id = "tab"))%>%
  autofit()%>%
  width(j = 1:4, width = c(2.5, 1.5, 1.5, 5)) %>%  # Ajuste les colonnes si besoin
  align(align = "center", part = "all") %>%
...
##  Interprétation : Stratégies d’adaptation et stress alimentaire

```{r interp_global_3}

# Créer le tableau de résumé pour les stratégies d’adaptation
tableau_strategies <- tribble(
  ~Indicateur,                              ~Femme,   ~Homme,   ~Interprétation,
  "SERS (Stratégies d’adaptation)",         "17 (±7)", "17 (±6)", "Scores similaires entre genres.",
  "rCSI (Score de stratégies réduites)",    "7 (±10)", "6 (±9)",  "Légèrement plus élevé chez les femmes, indiquant une adoption plus fréquente de stratégies d’adaptation.",
  "Urgence",                                "5.1%",    "5.0%",    "Aucune différence significative.",
  "Stress",                                 "22%",     "25%",     "Plus fréquent chez les hommes, ce qui peut refléter une pression socio-économique plus forte.",
  "Crise",                                  "6.2%",    "4.0%",    "Plus fréquent chez les femmes, ce qui pourrait refléter une plus grande vulnérabilité à l’insécurité alimentaire extrême."
)

# Générer la flextable
flextable(tableau_strategies) %>%
  set_caption("Comparaison globale des stratégies d’adaptation et des chocs selon le genre du chef de ménage",
 autonum = run_autonum(seq_id = "tab"))%>%
  autofit()
```
